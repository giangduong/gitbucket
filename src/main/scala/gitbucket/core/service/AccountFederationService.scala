...
trait AccountFederationService {
  ...
  def getOrCreateFederatedUser(
    ...
    getAccountByFederation(issuer, subject) match {
      ...
      case None =>
        val userName = preferredUserName.orElse(mailAddress.split("@").headOption).getOrElse("user")

        getAccountByUserName(userName) match {
          case Some(_) =>
            // Username exists, only create federation mapping
            createAccountFederation(issuer, subject, userName)
            getAccountByUserName(userName)

          case None =>
            // Username does not exist, create user then federation
  ...
  def createAccountFederation(issuer: String, subject: String, userName: String)(implicit s: Session): Unit = {
    val exists = AccountFederations
      .filter(af => af.issuer === issuer && af.subject === subject && af.userName === userName)
      .exists
      .run
